## Purchase reservation index migration

To prevent reservation conflicts, ensure the partial unique index on `purchases.scrapItemId` where `status='pending'` is applied. Run:

```
pnpm tsx scripts/migrate-purchase-index.ts
```

This drops any legacy full unique index and syncs the current partial unique index defined in the model.

## Chat payment flow request shape

The chat create-order request should be:

```
{ type: 'purchase', amount: <number>, referenceId: <conversationId>, email?: <string> }
```

The server resolves the item via the conversation.
## Payments (Razorpay)

To enable online payments for donations and marketplace purchases, configure these environment variables:
...
Flow overview:
- Create an order via `/api/razorpay/create-order` with `type` as `donation` or `purchase`.
- Open Razorpay Checkout using the returned `orderId` and `NEXT_PUBLIC_RAZORPAY_KEY_ID`.
- On success, call `/api/razorpay/verify-payment` to verify and complete donation/purchase.
- Webhook `/api/razorpay/webhook` provides server-side verification as a fallback.

## Conversations and Purchases

- Users can initiate a conversation from an item card; moderators/admins are included.
- Payment requests can be posted within the chat; buyers get a Pay button for Razorpay.
- Successful payment creates/completes a `Purchase` and marks the item as sold.

Index migration note:
- If you see reservation conflicts due to an older unique index on `purchases.scrapItemId`, run the migration before starting the server:

```powershell
pnpm tsx scripts/migrate-purchase-index.ts
```

- This drops the legacy unique index and re-syncs indexes to allow a single pending reservation per item (`partial unique { status: 'pending' }`).
- On startup, the server also attempts to drop the legacy index and sync indexes (`lib/server-startup.ts`).

## Mark as Sold (Admin)

Admins/moderators can mark an item as sold via a modal that captures:
- Sale price
- Channel: offline | chat | online
- Optional buyer fields

This calls `PATCH /api/protected/scrap-items/[id]`, which records a `Purchase`, updates the item, and closes a linked conversation.

## Barcode Scanner

Visit `/scanner` to scan item barcodes using the device camera. The app uses `@zxing/library` (ZXing) to decode barcodes/QRs.

## Notification Events

New domain notifications are emitted:
- `purchase_inquiry` when a buyer starts a conversation
- `payment_request` when staff requests payment
- `payment_completed` after successful payment
- `item_sold` when an item is marked sold

Helpers are in `lib/services/notification.service.ts` and are wired where needed.
