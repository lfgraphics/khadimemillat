...
export function EditCategoryDialog({ categoryId, categoryName, onCategoryUpdated }: EditCategoryDialogProps) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          defaultMonthlyAmount: category.defaultMonthlyAmount || 2000,
          active: category.active,
          sortOrder: category.sortOrder,
          eligibilityRules: {
            minAge: category.eligibilityRules?.minAge?.toString() || '',
            maxAge: category.eligibilityRules?.maxAge?.toString() || '',
            relationships: category.eligibilityRules?.relationships?.join(', ') || '',
            healthStatuses: category.eligibilityRules?.healthStatuses?.join(', ') || '',
            conditions: category.eligibilityRules?.conditions?.join(', ') || '',
            maritalStatuses: category.eligibilityRules?.maritalStatuses?.join(', ') || ''
          }
        });
      }
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the details for {categoryName}
          </DialogDescription>
        </DialogHeader>
